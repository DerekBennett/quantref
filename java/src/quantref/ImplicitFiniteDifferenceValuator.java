 ...
 * Time: 3:39:49 PM
 */
public class ImplicitFiniteDifferenceValuator implements Valuator<Option>{
    private StateOfTheWorld sotw;
    private int N=200;
    private int Nj=200;

    public ImplicitFiniteDifferenceValuator(StateOfTheWorld sotw, int timeDimension, int spaceDimension) {
        this.sotw = sotw;
    ...
    public Valuation getValuation(Option option) {
        ...
        for (int i=N-1; i>=0; i--) {
            solveImplicitTridiagonalSystem(V,pu,pm,pd,lambdaL,lambdaU);
            for(int j=0;j<=2*Nj;j++) {
    ...
    private void solveImplicitTridiagonalSystem(double[][] V, double pu, double pm, double pd, double lambdaL, double lambdaU) {
        // substitute boundary condition at j=0 in j=1
        double [] pmp = new double[2*Nj];
        double [] pp  = new double[2*Nj];
        pmp[1] = pm + pd;
        pp[1]  = V[0][1] + pd*lambdaL;

        // eliminate upper diagonal
        for (int j=2;j<2*Nj;j++){
            pmp[j] = pm - pu*pd/pmp[j-1];
            pp[j]  = V[0][j] - pp[j-1]*pd/pmp[j-1];
        }
